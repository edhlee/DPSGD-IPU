...
def make_vectorized_optimizer_class(cls):
  ...
  class DPOptimizerClass(cls):
    ...
    def __init__(
        self,
        l2_norm_clip,
        noise_std, effective_batch_size,
        num_microbatches=1,
      ...
      """
      super(DPOptimizerClass, self).__init__(momentum=0.9,*args, **kwargs)
    ...
    def compute_gradients(self,
      ...
      if callable(loss):
        ...
        # the number of microbatches = 1 by default (assuming batch size = 1)
        ...
        def process_microbatch(microbatch_loss):
          ...
          div = tf.cast(tf.maximum(global_norm /(self._l2_norm_clip), 1.),tf.float32)
          clipped_flat = [tf.cast(g / div,tf.float32) for g in grads_flat]
          clipped_grads = tf.nest.pack_sequence_as(grads_list, clipped_flat)
          return clipped_grads

        clipped_grads = process_microbatch( microbatch_losses)


        @tf.function
        def reduce_noise_normalize_batch(stacked_grads):
          if tf.rank(stacked_grads) > 0:
            summed_grads = stacked_grads # assumes microbatch_size = batch_size = 1
            noise = tf.random.normal(tf.shape(input=summed_grads),
                            stddev=self._noise_std*self._l2_norm_clip/np.sqrt(self._effective_batch_size),
                            dtype=tf.float32)
...
VectorizedDPSGD = make_vectorized_optimizer_class(MomentumOptimizer)
