    ...
    def compute_gradients(self,
      ...
      if callable(loss):
        ...
        def reduce_noise_normalize_batch(stacked_grads):
          if True or tf.rank(stacked_grads) > 0:
            summed_grads = stacked_grads
            noise = tf.random.normal(tf.shape(input=summed_grads), stddev=self._noise_std, dtype=tf.float32)
